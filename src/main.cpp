...
uint16_t plugin_unit_endpoint_id = 0;
...
// attribute) is checked if it matches plugin unit attribute. If yes, LED
// changes state to new one.
static esp_err_t on_attribute_update(attribute::callback_type_t type,
                                     uint16_t endpoint_id, uint32_t cluster_id,
                                     uint32_t attribute_id,
                                     esp_matter_attr_val_t *val,
                                     void *priv_data) {
  if (type == attribute::PRE_UPDATE && endpoint_id == plugin_unit_endpoint_id &&
...
void setup() {
  ...
  plugin_unit_endpoint_id = endpoint::get_id(endpoint_1);
...
get_onoff_attribute_value(esp_matter::attribute_t *attribute_ref) {
...
void set_onoff_attribute_value(esp_matter_attr_val_t *onoff_value,
                               uint16_t plugin_unit_endpoint_id) {
  attribute::update(plugin_unit_endpoint_id, CLUSTER_ID, ATTRIBUTE_ID,
                    onoff_value);
}
...
void loop() {
  if ((millis() - last_toggle) > DEBOUNCE_DELAY) {
    if (!digitalRead(TOGGLE_BUTTON_PIN)) {
      ...
      esp_matter_attr_val_t onoff_value =
          get_onoff_attribute_value(attribute_ref);
      onoff_value.val.b = !onoff_value.val.b;
      set_onoff_attribute_value(&onoff_value, plugin_unit_endpoint_id);
    }
  }
}
